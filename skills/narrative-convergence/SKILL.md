...
> The `Operator var` — Optional entity or theme filter (e.g. "Anthropic", "coordination markets"). If empty, scans all skill output categories.
...
The signal-category map is **operator-editable** and lives in `memory/skills/narrative-convergence/signal-categories.md`. If the file doesn't exist, create the seed below and continue. The categories are what let the skill measure *cross-category* diversity (the core of the convergence score) — edit them to match the skills you actually run.
...
If the `Operator var` is set, note it as a filter hint but still read all outputs — apply filtering at the scoring step.
...
If the `Operator var` is set, require the entity/theme to match the `Operator var` (substring, case-insensitive), or include it only if closely related.
...
Write `memory/skills/narrative-convergence/convergence-signals.md` (overwrite if exists):

```markdown
# Convergence Signals — Last Updated: today's date
...
---
*Generated by narrative-convergence on today's date. Top signal has N source skills across N categories.*
...
Otherwise, write to `.pending-notify-temp/narrative-convergence-today's date.md` (create the dir if needed):

```
narrative convergence — today's date
...
full breakdown: memory/skills/narrative-convergence/convergence-signals.md
```
...
- Do not write outside `output/narrative-convergence/` and `memory/skills/narrative-convergence/` plus today's log heading.
