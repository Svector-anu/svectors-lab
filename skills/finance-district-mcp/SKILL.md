...
> The `Operator var` — what to do with the wallet. Empty → a daily wallet brief (balances + notable price moves + top stablecoin yield). Or a specific instruction, e.g. `best USDC yield on Base`, `swap 5 USDC to ETH on Base`, `pay <x402-url> for <data>`.
...
3. **Act only on explicit instruction** — transfers, swaps, yield deposits, and x402 payments move real value. Do exactly what the `Operator var` asks, nothing more; sequence any irreversible action last, fail-closed. Amounts above the auto-approve threshold are rejected by the wallet — report that, never try to work around it.
4. **x402 paid calls** — follow the 402 flow (authorize within caps; gasless for the payer via EIP-3009).

   ```
   ### finance-district-mcp
   - Task: <the `Operator var`, or "daily brief">
...
- Do not write outside `output/finance-district-mcp/` and `memory/skills/finance-district-mcp/` plus today's log heading.
