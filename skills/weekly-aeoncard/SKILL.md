...
> The `Operator var` — window + delivery control.
...
3. **Derive the instance name + image link.** From the git origin, so the card self-labels and the notification deep-links the committed SVG.
   ...
   SVG="output/images/weekly-aeoncard-today's date.svg"
...
4. **Render the card + artifacts.** One call writes the SVG (canonical image), a best-effort PNG, the markdown report, the dashboard spec, and appends the run log. It prints a one-line JSON summary on stdout — capture it.
   ...
   SUMMARY="$(python3 skills/weekly-aeoncard/render_card.py \
     --csv "$CSV" --today "today's date" --window-days "$WINDOW_DAYS" \
     --instance "$INSTANCE" --img-link "$IMG_LINK" \
     --out "$SVG" \
     --png "output/images/weekly-aeoncard-today's date.png" \
     --report "output/articles/weekly-aeoncard-today's date.md" \
     --dashboard "apps/dashboard/outputs/weekly-aeoncard.json" \
     --log "memory/logs/today's date.md")"
   RC=$?
   if [ "$RC" -ne 0 ]; then
     exit 0
   fi
...
6. **Log.** The render step already appended a `### weekly-aeoncard` block to `memory/logs/today's date.md` (window, weekly + all-time totals, top skills, image path, `WEEKLY_AEONCARD_OK`). Do not duplicate it.
...
- **Idempotent.** Same-day reruns overwrite `output/images/weekly-aeoncard-today's date.svg`, the report, and the dashboard spec; the log block appends (one per run) so reruns show drift.
- **The SVG is the deliverable.** The PNG is a convenience for chat/social; its absence is never an error.

## Do not

- Do not write outside `output/weekly-aeoncard/` and `memory/skills/weekly-aeoncard/` plus today's log heading.
