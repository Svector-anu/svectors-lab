...
> The `Operator var` — selects the mode and scope:
...
**Parse the `Operator var` to pick the branch** (trim whitespace, compare case-insensitively):
- If the `Operator var` starts with `revise:` — run the **Revise branch** (below) and stop. This is the shape `scripts/telegram-route.sh` sends when the operator replies to a "refine these replies?" force-reply prompt; catch it before mode parsing.
- If the `Operator var` is `from-logs` or `--from-logs` — optionally followed by a whitespace-separated `@handle` or project name — run **Mode B (From-Logs Engagement)**. Treat any trailing token as an optional filter that narrows the opportunity scan to that handle/project.
- Otherwise run **Mode A (Reply Drafting)**, treating the `Operator var` as the scope: empty, `@handle`, numeric X list ID, or a topic string.
...
If `KEY_PRESENT` (it will be), Path A is required. Build the payload **file** `/tmp/xai-rm-payload.json` per the `Operator var` (three shapes below — each branch writes the same fixed file), then:
...
**The payload file `/tmp/xai-rm-payload.json` depends on the `Operator var`.** Whichever branch matches, build it with `jq -n --arg` (never a shell-interpolated string) and write it to that one fixed path — the `./secretcurl` call above then sends it with `-d @/tmp/xai-rm-payload.json`:

**If the `Operator var` looks like an X list ID** (numeric):
```bash
LIST_ID="the `Operator var`"
...
**If the `Operator var` looks like a `@handle`** — same query intent, scoped to that handle's recent original posts:
```bash
HANDLE="the `Operator var`"
...
**If the `Operator var` is a topic** (or empty) — same query intent with the `Operator var` (or the top 2–3 topics from `memory/MEMORY.md` when empty) as the search query. When empty, also pull tweet candidates surfaced in the last 2 days of `tweet-roundup` and `list-digest` logs as a backup pool.
```bash
TOPIC="the `Operator var`"   # when empty, substitute the top 2–3 topics from memory/MEMORY.md
...
Append to `memory/logs/today's date.md` under the shared `### reply-maker` heading (see **Log** below), using the **Mode A** template.
...
**Projects-of-interest list:** if `memory/skills/reply-maker/projects-of-interest.md` exists, treat the project names listed there as the things to watch for mentions, cosigns, attributions, and fork moments. If the file is missing or empty, fall back to any project names that appear in recent logs or in MEMORY.md. If a filter token was passed (`from-logs @handle` or `from-logs <project>`), narrow the scan to opportunities involving that handle/project.
...
```
*Reply Maker (from-logs) — today's date*
...
Append to `memory/logs/today's date.md` under the shared `### reply-maker` heading (see **Log** below), using the **Mode B** template.
...
1. **Persist the drafts** to a stable path a later `revise:` run can reload:
   ...
   **Dedup — once per produced draft.** Before offering, scan the last ~2 days of `memory/logs/` for a `FORCE_REPLY_OFFERED: revise` line dated today's date; if present, skip the offer. When you send it, append the marker under the run's `### reply-maker` entry:
   ```
...
Append one entry to `memory/logs/today's date.md` under a single `### reply-maker` heading, with a `**Mode:**` discriminator line naming which branch ran.
...
- Do not write outside `output/reply-maker/` and `memory/skills/reply-maker/` plus today's log heading.
