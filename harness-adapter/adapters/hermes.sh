...
ARGS=(-z --usage-file "$RH_TMPDIR/hermes-usage.json"); [ -n "${RH_MODEL:-}" ] && [ "$RH_MODEL" != "default" ] && ARGS+=(--model "$RH_MODEL"); [ "${RH_MODE:-write}" = "read-only" ] && ARGS+=(--safe-mode); [ -z "${HERMES_AUTH:-}" ] && [ -n "${OPENROUTER_API_KEY:-}" ] && ARGS+=(--provider openrouter)
OUT="$RH_TMPDIR/hermes-out.txt"; hermes "${ARGS[@]}" "$PROMPT" > "$OUT"; rc=$?
