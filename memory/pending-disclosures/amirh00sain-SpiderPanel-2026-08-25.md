...
`main.py` initializes the admin password and secret from environment variables with documented defaults:
...
Suggested fixes:
- Refuse to start, or force a password reset, when the default `ADMIN_PASSWORD` is detected.
- Generate and persist a random `SECRET_KEY` on first startup if one is not provided.
- Replace single-round SHA256 with bcrypt, argon2, or scrypt.
...
Suggested fixes:
- Set `secure=True, samesite="Strict"` on the session cookie.
- Restrict `allow_origins` to the panel's actual public domain(s), or remove `allow_credentials=True` if wildcard origins are required.
