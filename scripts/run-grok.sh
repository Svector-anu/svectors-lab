...
# exit 0 either way (existing contract, see scripts/tests/test_run_grok.sh case 3d:
# a refresh failure is not treated as fatal here, since the on-disk access token may
# still have some life left). but "auth staged" is a lie when we just warned that
# refresh failed - that false-positive log line is what let this go unnoticed for
# 11 days (ISS-005): every run "succeeded" at setup right before dying downstream on
# an already-expired token, so nothing ever pointed at auth as the cause.
