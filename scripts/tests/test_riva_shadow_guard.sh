#!/usr/bin/env bash
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
WF="$ROOT/.github/workflows/aeon.yml"
SKILL="$ROOT/skills/vuln-scanner/SKILL.md"
CAPS="$ROOT/scripts/resolve-riva-capabilities.sh"

grep -q 'resolve-riva-capabilities.sh' "$WF"
grep -q 'SHADOW_MODE=1' "$CAPS"
grep -q 'SKILL_MODE=read-only' "$CAPS"
grep -q 'SHADOW_MODE.*!=.*1' "$WF"
grep -q 'unset ALL_SECRETS GH_GLOBAL GH_TOKEN GITHUB_TOKEN' "$WF"
grep -q 'If `KERNEL=shadow`' "$SKILL"
grep -q 'do not execute A5' "$SKILL"

TMP="$(mktemp -d -t riva-shadow-guard.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
for selector in shadow shadow:owner/repo compare compare:owner/repo; do
  GITHUB_ENV="$TMP/github-env" SKILL_NAME=vuln-scanner SKILL_VAR="$selector" \
    bash -c 'source "$1"; [ "$SHADOW_MODE" = 1 ] && [ "$SKILL_MODE" = read-only ]' _ "$CAPS"
done

GITHUB_ENV="$TMP/github-env" SKILL_NAME=vuln-scanner SKILL_VAR=riva:owner/repo \
  bash -c 'source "$1"; [ "$SHADOW_MODE" = 0 ] && [ "$SKILL_MODE" = write ]' _ "$CAPS"
echo "ok - Riva shadow mode is workflow-isolated and comparison-only"
