...
# 3d2. poc for the ISS-005 silent-failure bug: rc=0 is correct (3d, unchanged) but
# the setup-complete line must not claim "staged" when we just warned refresh failed
# - that false-positive is what let this run failed 21/21 times for 11 days with
# nothing pointing at auth as the cause. reusing the same invalid_grant fixture as 3d.
