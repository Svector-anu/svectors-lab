...
**Live behavioral proof, not just code review.** `pr-review` verifies a diff — it reads code, runs whatever checks CI already runs, and posts a SHA-bound verdict. Confirmed live 2026-09-18 ([run 35332398314](https://github.com/Svector-anu/svectors-lab/actions/runs/35332398314), PR #87: `approve-ready — no blockers` at the exact merged SHA) that this works and is not a rubber stamp — the same day, `dev-loop` correctly refused to close two other runs where re-review still found a real issue or the repair guard's precondition wasn't met, rather than claiming false success. But nothing in the loop launches the actual application and drives the feature it just built — no browser, no CLI invocation of the new behavior, no captured screenshot or command output as evidence beyond "the code looks right and CI is green." For most of this repo's own skills (Markdown prompts executed by a headless agent, not a UI), that gap is real but narrower than it would be for a typical web app. Not started. See `docs/prd/proposed-epoch-rebrand.md` for the fuller writeup of this gap under the name "Prove."

## Engineering notes

**2026-09-18: `feature` now reads the feature map before building.** Fixed same-day: `feature/SKILL.md` had zero references to `docs/product/feature-map.md`, so it could duplicate or silently contradict something already marked `shipped`/`in-progress` there. One-line fix, verified via a live `pr-review` dispatch against the PR that made the change (not just CI), merged as [#87](https://github.com/Svector-anu/svectors-lab/pull/87).
